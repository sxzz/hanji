...
export const zhTW: typeof zhCN = {
  ...
  hero: {
    title: '一個漢字四種寫法',
    body: '中國大陸、香港、臺灣、日本，同一個字寫出來常常不一樣。',
    split: '拆開看',
    merge: '疊回去',
  },
  nav: {
  ...
  about: {
    ...
    limit5:
      'OpenCC與各地規範字表衝突時以規範字表為準：臺灣《常用國字標準字體表》收脣與祕，OpenCC卻把它們轉成唇、秘，那兩欄就寫規範表的形。跨碼位的對應關係來自OpenCC。它的異體表裡混著兩種關係：地區寫法不同的同一個字（裏/裡），和一個字代用另一個字（臺灣用著兼表著與着，日本用欠代缺、缶代罐）。後者若照反，被代用的那個現行漢字就會被吞掉，所以判據是「某地區的字表分開收錄了這兩個字，就不合併」——著、欠、缶、糸都因此保住了自己的條目，並與代用它的那一組互相連結。',
    notice1:
      '這裡的資料不是權威資料。漢字的收錄、字形、筆畫、讀音，各地標準之間本來就有分歧，也不存在一份能讓所有人滿意的資料。',
    notice2:
      '為了不讓這裡變成一個需要長期人工維護的字表，本專案盡量貼著已有的公共資料走——Unicode Unihan、各地文化教育主管機關發布的規範字表、Adobe與Google公開的字體資源——原樣取用。判定規則都寫在上面，沒有個人取捨，也不代表任何立場。',
    notice3:
      '正式場合請以原始規範與辭典為準。每個字的詳情頁都列了該地區自己的字典，可以逐條對照。',
