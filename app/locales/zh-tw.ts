...
export const zhTW: typeof zhCN = {
  ...
  about: {
    ...
    method2:
      '最終判定取黑體與宋體結果的聯集：只要其中一款把兩地畫成同一字形，本站就按同形處理。這能排除只出現在單款字體中的設計細節。例如Source Han Sans為約五分之一的日本常用漢字提供獨立字形，其中約兩百個在Source Han Serif並未區分，像了、人、子、水、金；本站不把這類差異算作地區規範差異。',
    method3:
    ...
    limitPrint:
      '本站只比較通用黑體與宋體中的印刷字形，不涵蓋手寫習慣，也不以教科書體的示範字形為準。日語教科書體主要為日語教學設計，並沒有與中、港、臺、韓共用同一字形池的正式地區版本；若拼接風格相近但來源不同的字體，地區差異與字體自身的設計差異就無法分開。為了控制變因，本站只能選用同時提供五地版本的同源字體系列。',
    limit1:
    ...
    notice1:
      '本站是依據公開資料製作的字形對照工具，不是各地的規範、辭典或教學材料。頁面呈現的是本站採用的資料與自動規則所得結果，不能據此斷定某個字在當地只有這一種「正確」形式。',
    notice2:
      '各地規範的適用範圍與定義不盡相同，字體也只是規範的一種設計實作。本站會整理、轉換並合併不同來源的資料，也會為未收錄項目補上參考字形；這些都是工程取捨，難免帶來簡化、遺漏與錯誤。',
    notice3:
