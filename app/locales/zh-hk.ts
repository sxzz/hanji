...
export const zhHK: typeof zhCN = {
  ...
  filter: {
    ...
    glyph: '字形',
    cp: '碼點',
    glyphHint: '比較印刷字形。五地使用同一碼點而字形不同，也算有差異。',
  ...
  about: {
    ...
    license: '授權',
    method1:
      '判定依據Adobe Source Han Sans與Source Han Serif；頁面則使用與它們同源的Noto Sans CJK與Noto Serif CJK顯示結果。每套字體讓中、港、台、日、韓五個地區共用一個字形池，並分別提供「Unicode碼點→字形編號（CID）」對應。兩地對應到同一CID，就視為同形。',
    method2:
      '最終判定取黑體與宋體結果的並集：只要其中一款把兩地畫成同一字形，本應用就按同形處理。這能排除只出現在單款字體中的設計細節。例如Source Han Sans為約五分之一的日本常用漢字提供獨立字形，其中約兩百個在Source Han Serif並未區分，像了、人、子、水、金；本應用不把這類差異算作地區規範差異。',
    method3:
    ...
    scope2:
      '其中{identical}行五地字形完全一致，{allDiffer}行五地各不相同。字形相同的字同樣收錄——這裏是五地漢字的字表，字形差異只是其中一個維度。',
    scope3:
      '台灣《次常用國字表》只為已有行提供二級收錄狀態和候選，不參與生成新行；其6,343個主條目中有3,599個獨有條目明確在產品範圍之外。',
    limitPrint:
      '本應用只比較通用黑體與宋體中的印刷字形，不涵蓋手寫習慣，也不以教科書體的示範字形為準。日語教科書體主要為日語教學設計，並沒有與中、港、台、韓共用同一字形池的正式地區版本；若拼接風格相近但來源不同的字體，地區差異與字體自身的設計差異就無法分開。為了控制變數，本應用只能選用同時提供五地版本的同源字體系列。',
    limit1:
    ...
    limit6:
      '每個地區格會區分字表主條目、括注異體和未收錄回退。未收錄格仍顯示傳承參考字形；這些狀態只在詳情頁說明，不在列表頁增加角標。若明確對應到Noto未涵蓋的碼點，頁面仍會顯示該碼點，並分別使用Plangothic P1（黑體）和WenJin Mincho P2（宋體）的專案內置子集補全；這些補充字形不參與地區差異判定。',
    notice1:
      '本應用是根據公開數據製作的字形對照工具，不是各地的規範、詞典或教材。頁面顯示的是本應用採用的數據與自動規則所得結果，不能據此斷定某個字在當地只有這一種「正確」形式。',
    notice2:
      '各地規範的適用範圍和定義不盡相同，字體也只是規範的一種設計實現。本應用會整理、轉換及合併不同來源的數據，也會為未收錄項目補上參考字形；這些都是工程取捨，難免帶來簡化、遺漏和錯誤。',
    notice3:
      '頁面中的「同形」或「不同」只在上述數據、字體與規則內成立；地區的排列和分組只為方便對照，不表示優劣或立場。正式場合請以原始規範與詞典為準；每個字的詳情頁都列有相應地區的字典連結。',
    notice4: '漢字數量龐大，出錯難免。發現數據有誤，請透過{issues}回報。',
    data1:
      '本應用的字表數據以JSON提供，並已開放CORS跨來源存取；在遵守下列授權與第三方條款的前提下，可透過fetch、XHR、直接連結等方式自由取用：',
    licenseCode:
      '本項目的程式碼、介面實作與項目原創文件採用{mit}授權；完整的授權範圍與名稱使用規則見{licenseFile}。',
    licenseData:
      '除另有註明外，由漢智原創的數據庫結構、數據選擇與編排及原創中繼數據採用{cc}授權。',
    licenseThirdParty:
