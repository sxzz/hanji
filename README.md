...
需要调试 PWA 安装体验时，用 `pnpm dev:pwa` 启动后在 Chromium 中打开 `http://localhost:3000`。开发 Service Worker 使用 Network Only 导航且不下载生产环境的完整离线缓存，因此不会干扰 HMR；完整离线行为仍应使用 `pnpm generate` 后的产物验证。开发模式关闭安装条只对当前页面有效，刷新即可重复测试，也不会污染生产预览的关闭状态。若浏览器仍控制着修改前的开发 Service Worker，请在 DevTools 的 Application 面板更新或注销它，再刷新一次。
