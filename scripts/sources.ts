...
export const SOURCES: Source[] = [
  ...
  },
  {
    ...
    note: {
      'zh-CN':
        '仅提取本站字组引用的中、台、日、韩字形，保留原始笔画轮廓与中线，并按字组主键转换成32个哈希分片；动画以官方示例的轮廓裁剪方式绘制。同一字组切换地区只需一个分片；轮廓与中线完全一致的地区仅存一份变体，界面也合并成一个选项。香港按本站的Source Han字形分组复用已有数据，优先顺序为台湾、中国大陆、日本、韩国。分片与随附授权由构建脚本生成至/strokes，不提交到仓库；修改后的AnimCJK数据继续按APL提供。',
      'zh-TW':
        '只擷取本站字組引用的中、臺、日、韓字形，保留原始筆畫輪廓與中線，並依字組主鍵轉換成32個雜湊分片；動畫採官方範例的輪廓裁剪方式繪製。同一字組切換地區只需一個分片；輪廓與中線完全一致的地區只儲存一份變體，介面亦合併成單一選項。香港依本站的Source Han字形分組沿用既有資料，優先順序為臺灣、中國大陸、日本、韓國。分片與隨附授權由建置腳本產生至/strokes，不提交至儲存庫；修改後的AnimCJK資料繼續依APL提供。',
      'zh-HK':
        '只抽取本站字組引用嘅中、台、日、韓字形，保留原始筆畫輪廓同中線，並按字組主鍵轉換成32個雜湊分片；動畫用官方範例嘅輪廓裁剪方式繪製。同一字組切換地區只需要一個分片；輪廓同中線完全一致嘅地區只儲存一份變體，介面亦合併成一個選項。香港按本站嘅Source Han字形分組沿用已有數據，優先次序係台灣、中國大陸、日本、韓國。分片同附帶授權由建置腳本產生到/strokes，唔會提交到儲存庫；修改後嘅AnimCJK數據繼續按APL提供。',
      'ja-JP':
        '本サイトの字グループが参照する中国・台湾・日本・韓国の字形だけを抽出し、元の筆画輪郭と中心線を保持したまま、字グループキーをハッシュした32個のシャードへ変換します。アニメーションは公式サンプルと同じ輪郭クリッピング方式で描画し、同じ字グループ内の地域切替は1シャードだけで済みます。輪郭と中心線が完全に一致する地域は1つのバリアントだけを保存し、画面上でも1つの選択肢にまとめます。香港は本サイトのSource Han字形グループに基づいて既存データを流用し、台湾、中国大陸、日本、韓国の順に優先します。シャードと付属ライセンスはビルドスクリプトが/strokesへ生成し、リポジトリにはコミットしません。変更後のAnimCJKデータもAPLで提供します。',
      'ko-KR':
        '이 사이트의 글자 그룹이 참조하는 중국·대만·일본·한국 자형만 추출하고 원본 획 윤곽과 중심선을 유지한 채 글자 그룹 키를 해시한 32개 샤드로 변환합니다. 애니메이션은 공식 예제와 같은 윤곽 클리핑 방식으로 그리며 같은 글자 그룹의 지역 전환은 샤드 하나만 사용합니다. 획 윤곽과 중심선이 완전히 같은 지역은 변형 하나만 저장하고 화면에서도 하나의 선택 항목으로 합칩니다. 홍콩은 이 사이트의 Source Han 자형 그룹에 따라 기존 데이터를 재사용하며 대만, 중국 대륙, 일본, 한국 순으로 우선합니다. 샤드와 동봉 라이선스는 빌드 스크립트가 /strokes에 생성하며 저장소에는 커밋하지 않습니다. 수정한 AnimCJK 데이터도 APL로 제공합니다.',
    },
  },
  {
    id: 'hanzi-chars',
